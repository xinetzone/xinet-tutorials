...
要将项目添加到场景，首先需要构造一个 QGraphicsScene 对象。然后，您有两个选择：通过调用 `addItem()` 添加现有的 QGraphicsItem 对象，或者可以调用下列便捷函数之一 `addEllipse()`，`addLine()`，`addPath()`，`addPixmap()`，`addPolygon()`，`addRect()` 或 `addText()`，它们都返回一个指向新添加项目的指针。使用这些函数添加的图元的维度是相对于图元（item）的坐标系的，并且图元的位置在场景中被初始化为 $(0,0)$。
...
QGraphicsScene 还管理某些图元项目状态，例如图元项目选择和焦点。要选择项目，请调用 `setSelectionArea()`（传递任意形状来选择场景中的项目），并要清除当前选择，请调用`clearSelection()`。调用 `selectedItems()` 以获取所有选定项的列表。

函数 `setSelectionArea()` 还用作 QGraphicsView 中橡皮筋选择的基础。要获取所有当前选定项目的列表，请调用 `selectedItems()` 。QGraphicsScene处理的另一个状态是项目是否具有键盘输入焦点。您可以通过调用 `setFocusItem()` 或 `setFocus()` 来设置焦点，或者通过调用 `focusItem()` 来获取当前焦点。
...
最后，QGraphicsScene 允许您通过 `render()` 函数将场景的一部分渲染到绘制设备中。
...
该例子使用了 `QtWidgets.QGraphicsItem.setPos(pos)` 方法，将图元项目的位置设置为 `pos`（在父坐标中）。 对于没有父项的项目，`pos` 位于场景坐标中。其中的参数 `pos` 可以是 `QPointF` 或者 `(x, y)`。
...
![图5 一个多边形元的例子](images/polygen.png)
...
![图6 一个椭圆元的例子](images/ellipse.png)
...
![图8 一个简单的纯文本元的例子](images/plain_text.png)
...
![图9 一个简单的富文本元的例子](images/rich_text.png)


## 4 Graphics View 场景与视角

不使用使用 `QtWidgets.QGraphicsScene.addItem` 添加图元，可以直接使用特定的图元创建函数添加图元。
...
![图10 修改 scene 中文本位置](images/text_pos.png)
...
![图11 展示不同的 View，不同的 Scene](images/multi_view_scene.png)
...
![图12 展示相同的 Scene，不同的 View](images/multi_view.png)
...
def main():
    ...
    scene.addPixmap(QPixmap(r'D:\xinetzone\images\test.jpg'))
    grview.setScene(scene)
    item = GraphicsRectItem(20, 20, 300, 150)
    item1 = GraphicsRectItem(200, 20, 300, 50)
    scene.addItem(item)
    scene.addItem(item1)
